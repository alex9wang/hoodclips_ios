...
- (void) initView{
    ...
    self.opacityView.backgroundColor = [SlideMenuOptions sharedInstance].opacityViewBackgroundColor;
    ...
    leftFrame.size.width = [SlideMenuOptions sharedInstance].leftViewWidth;
    ...
    rightFrame.size.width = [SlideMenuOptions sharedInstance].rightViewWidth;
...
- (void) handleLeftPanGesture:(UIPanGestureRecognizer*)panGesture{
    ...
    switch (panGesture.state) {
        case UIGestureRecognizerStateBegan:
            [LeftPanState sharedInstance].frameAtStartOfPan = self.leftContainerView.frame;
            [LeftPanState sharedInstance].startPointOfPan = [panGesture locationInView:self.view];
            [LeftPanState sharedInstance].wasOpenAtStartOfPan = [self isLeftOpen];
            [LeftPanState sharedInstance].wasHiddenAtStartOfPan = [self isLeftHidden];
            
            [self.leftViewController beginAppearanceTransition:[LeftPanState sharedInstance].wasHiddenAtStartOfPan animated:YES];
        ...
        case UIGestureRecognizerStateChanged:
        {
            ...
            self.leftContainerView.frame = [self applyLeftTranslation:translation toFrame:[LeftPanState sharedInstance].frameAtStartOfPan];
        ...
        case UIGestureRecognizerStateEnded:
        {
            ...
            if (panInfo.actionSlide == OPEN) {
                if (![LeftPanState sharedInstance].wasHiddenAtStartOfPan) {
...
- (void) handleRightPanGesture:(UIPanGestureRecognizer*)panGesture{
    ...
    switch (panGesture.state) {
        case UIGestureRecognizerStateBegan:
            [RightPanState sharedInstance].frameAtStartOfPan = self.rightContainerView.frame;
            [RightPanState sharedInstance].startPointOfPan = [panGesture locationInView:self.view];
            [RightPanState sharedInstance].wasOpenAtStartOfPan = [self isRightOpen];
            [RightPanState sharedInstance].wasHiddenAtStartOfPan = [self isRightHidden];
            
            [self.rightViewController beginAppearanceTransition:[RightPanState sharedInstance].wasHiddenAtStartOfPan animated:YES];
        ...
        {
            CGPoint translation = [panGesture translationInView:panGesture.view];
            self.rightContainerView.frame = [self applyRightTranslation:translation toFrame:[RightPanState sharedInstance].frameAtStartOfPan];
        ...
        case UIGestureRecognizerStateEnded:
        {
            ...
            if (panInfo.actionSlide == OPEN) {
                if (![RightPanState sharedInstance].wasHiddenAtStartOfPan) {
...
- (void) openLeftWithVelocity:(CGFloat)velocity{
    ...
    NSTimeInterval duration = [SlideMenuOptions sharedInstance].animationDuration;
    if (velocity != 0.0) {
    ...
    [UIView animateWithDuration:duration delay:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        if (self) {
            ...
            self.opacityView.layer.opacity = [SlideMenuOptions sharedInstance].contentViewOpacity;
            self.mainContainerView.transform = CGAffineTransformMakeScale([SlideMenuOptions sharedInstance].contentViewScale, [SlideMenuOptions sharedInstance].contentViewScale);
        }
...
- (void) changeLeftViewWidth:(CGFloat) width{
    [SlideMenuOptions sharedInstance].leftViewWidth = width;
    CGRect leftFrame = self.view.bounds;
...
- (void) changeRightViewWidth:(CGFloat) width{
    [SlideMenuOptions sharedInstance].rightBezelWidth = width;
    CGRect rightFrame = self.view.bounds;
...
- (CGFloat)leftMinOrigin{
    return -[SlideMenuOptions sharedInstance].leftViewWidth;
}
...
- (PanInfo)panLeftResultInfoForVelocity:(CGPoint)velocity{
    ...
    CGFloat pointOfNoReture = floor([self leftMinOrigin]) + [SlideMenuOptions sharedInstance].pointOfNoReturnWidth;
...
- (PanInfo)panRightResultInfoForVelocity:(CGPoint) velocity{
    CGFloat thresholdVelocity = -1000.0;
    CGFloat pointOfNoReture = floor(CGRectGetWidth(self.view.bounds)) - [SlideMenuOptions sharedInstance].pointOfNoReturnWidth;
...
- (void) applyLeftOpacity{
    ...
    CGFloat opacity = [SlideMenuOptions sharedInstance].contentViewOpacity * openedLeftRatio;
    self.opacityView.layer.opacity = opacity;
}

- (void) applyRightOpactiy{
    CGFloat openedRightRatio = [self getOpenedRightRatio];
    CGFloat opacity = [SlideMenuOptions sharedInstance].contentViewOpacity * openedRightRatio;
    self.opacityView.layer.opacity = opacity;
}

- (void) applyLeftContentViewScale{
    CGFloat openedLeftRatio = [self getOpenedLeftRatio];
    CGFloat scale = 1.0 - ((1.0 - [SlideMenuOptions sharedInstance].contentViewScale) * openedLeftRatio);
    self.mainContainerView.transform = CGAffineTransformMakeScale(scale, scale);
}

- (void) applyRightContentViewScale{
    CGFloat openedRightRatio = [self getOpenedRightRatio];
    CGFloat scale = 1.0 - ((1.0 - [SlideMenuOptions sharedInstance].contentViewScale) * openedRightRatio);
    self.mainContainerView.transform = CGAffineTransformMakeScale(scale, scale);
}

- (void) addShadowToView:(UIView*) targetContainerView{
    targetContainerView.layer.masksToBounds = NO;
    targetContainerView.layer.shadowOffset = [SlideMenuOptions sharedInstance].shadowOffset;
    targetContainerView.layer.shadowOpacity = [SlideMenuOptions sharedInstance].shadowOpacity;
    targetContainerView.layer.shadowRadius = [SlideMenuOptions sharedInstance].shadowRadius;
...
- (void) setOpenWindowLevel {
    if ([SlideMenuOptions sharedInstance].hideStatusBar) {
...
- (BOOL) gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRecognizeSimultaneouslyWithGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer{
    return [SlideMenuOptions sharedInstance].simultaneousGestureRecognizers;
}

- (BOOL) slideLeftForGestureRecognizer:(UIGestureRecognizer*)gesture point:(CGPoint)point{
    return [self isLeftOpen] || ([SlideMenuOptions sharedInstance].panFromBezel && [self isLeftPointContainedWithinBezelRect:point]);
}


- (bool) isLeftPointContainedWithinBezelRect:(CGPoint)point{
    CGFloat bezelWidth = [SlideMenuOptions sharedInstance].leftBezelWidth;
    if (bezelWidth) {
...
- (bool) slideRightViewForGestureRecognizer:(UIGestureRecognizer*)gesture point:(CGPoint)point{
    return [self isRightOpen] || ([SlideMenuOptions sharedInstance].rightPanFromBezel && [self isRightPointContainedWithinBezelRect:point]);
}


- (bool) isRightPointContainedWithinBezelRect:(CGPoint)point{
    CGFloat rightBezelWidth = [SlideMenuOptions sharedInstance].rightBezelWidth;
    if (rightBezelWidth) {
