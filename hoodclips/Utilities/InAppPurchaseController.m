...
+ (InAppPurchaseController*)sharedController {
    static InAppPurchaseController* sController = nil;
    if (sController == nil) {
        sController = [InAppPurchaseController new];
    }
    return sController;
}
...
- (id)init {
    self = [super init];
    if (self) {
        productIdentifierList = @[kIAPItemProductId199];
...
- (void)provideContentForProductIdentifier:(NSString *)productIdentifier restore:(BOOL)restore {
    ...
    ////
    [[NSNotificationCenter defaultCenter] postNotificationName:IAPHelperProductPurchasedNotification object:nil userInfo:@{IAPHelperProductPurchasedNotification: productIdentifier}];
}
