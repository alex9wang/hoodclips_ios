...
- (id)showDropDown:(UIButton *)b:(CGFloat *)height:(NSArray *)arr:(NSArray *)imgArr:(NSString *)direction {
    ...
    if (self) {
        ...
        if ([direction isEqualToString:@"up"]) {
            self.frame = CGRectMake(btn.origin.x - 100, btn.origin.y-*height, btn.size.width * 10, *height);
        } else if([direction isEqualToString:@"down"]) {
            self.frame = CGRectMake(btn.origin.x - 100, btn.origin.y+btn.size.height, 140, *height);
        }
        self.table.frame = CGRectMake(0, 0, 140, *height);
